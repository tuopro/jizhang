...
- 最新源码已推送至原 GitHub 仓库 `tuopro/jizhang` 的 `main`，源码提交为 `a709cd22081df52c1a6609d8dbc38997de0b3491`。本次包含企业成员邀请小程序码、二维码按钮打包修复、真实WXML渲染测试及报告，共22个文件；上传前656/656测试、24页原生编译和暂存差异检查通过。Git源码同步已完成，不代表再次部署ledger或更新微信体验版。

- 二维码入口修复结果见 `docs/邀请二维码入口未显示排查与修复报告.md`。开发者工具原目录复现旧页面，清编译缓存后暴露 services/invite-qrcode.js 未进入包；新增该文件的精确packOptions.include，并刷新编译/文件列表缓存后，已在当前真实admin+active invite页面观察到二维码按钮。本轮未改成员页或云函数；656/656测试、24页原生编译通过。用户反馈此前已部署ledger并上传体验版；本次修复未再次部署/上传，手机旧体验包需后续重新上传前端才会更新。

- 此前完成“企业成员邀请小程序码功能”，见 `docs/企业成员邀请小程序码功能报告.md`。企业成员页支持二维码预览/保存；与微信卡片共用原邀请，7天、多成员复用、撤销、停用和member默认角色不变。原48位hex token超过scene限制，按需在原member_invites上加32位base64url qrSceneCode，保留192位随机强度，不迁移历史数据。651/651测试及24页原生编译通过；未部署、未上传体验版、未审核、未修改正式数据/存储权限、未提交Git。

- 此前源码已于2026-09-22推送至现有公开 GitHub 仓库 `tuopro/jizhang` 的 `main`。源码提交为 `cb39e4c71103ceada7300b00562a29925424578c`，包括自定义规格创建权限、底槽识别、Excel 即时清理与48小时GC，共38个变更文件；同时推送了进度记录 `cffc36f98cd231b8f3bfbe4c4a63281c8f8da077`。此前公开推送审批拦截已在用户明确确认后解决；GitHub同步不代表微信上线。
...
- 自定义规格创建由真实 active admin、当前客户负责人或该客户唯一真实开放账期负责人授权；普通成员必须提供业务客户，由服务端事务重读成员/客户/开放期。产品仍为企业共享，不持久化客户归属。多开放期仅关闭账期负责人分支；重复创建复用已有产品时普通成员不得补写任何属性。全局产品管理和修改/启停继续仅管理员。
- “底槽”是明确产品身份，不映射为底板/底座/槽底/无盖/开槽。名称、specialTags、别名匹配、特殊长度及重复检测都需保留底槽边界；普通规格和底槽不能自动互相匹配，已存同身份底槽按结构化属性复用。
...
## 2026-09-22 Excel 临时云文件即时清理与48小时GC（本地完成，未部署）

- 原流程已在下载finally中尝试cleanup，再用本机tempFilePath打开/微信转发；无需用户另做手工下载。不能从本地源码确定9月21日三份真实残留的具体原因，未读取当时云日志或核对线上部署版本，不把可能原因记为已证实。
- 本地确认并修复：finally中的页面进度回调抛错可跳过清理、无清理等待超时/脱敏失败日志、服务端无有效delete结果仍误报成功、不存在文件非幂等。现最多两次、每次等待6秒；失败不影响本机Excel，不显示内部清理错误。页面退出后仍尽力清理，且不再更新退出页面或自动打开。
- 即时cleanup沿用真实OPENID→active membership→服务端tenant，严格核对完整生成路径、tenant/member hash及可用的真实环境ID，拒绝其它成员/企业/目录/图片/路径穿越。按实际fileID确认删除结果；ledger入口、action白名单和其它权限不变。
- 新增独立statement-export-gc及每小时触发器本地配置。固定statement-exports/、项目生成xlsx和真实LastModified≥48h，删除前HEAD再查时间/ETag；每页100、每批20、单次最多扫描1000/尝试删除200、40秒软预算。忽略event所有范围/年龄参数，真实客户端OPENID调用拒绝。
- 用固定云存储对象statement-exports/.gc-cursor.json保存分页进度，跨冷启动续跑；完整扫描后从头覆盖失败和新文件，不新增数据库集合/字段。正常无积压时通常48至49小时兜底，持续故障或积压会延后；不能保证进程被杀时客户端finally继续运行。
- 官方Manager SDK支持SCF运行时临时身份；新GC包锁定@cloudbase/manager-node 5.8.8和wx-server-sdk 4.0.2及独立lockfile。强制运行时session token，无本机/长期密钥回退。尚未验证正式环境的凭证注入、列对象/HEAD/删除/游标权限，缺失时关闭清理并记录脱敏code。
- 新增62项测试，npm test 450/450通过；npm run check通过24页面、107 JS、24 WXML、25 WXSS微信原生编译；git diff --check通过。177文件基线最终核验中170个一致，6个已有源码/测试/检查文件及本偏好进度文档按范围调整；基线位于/private/tmp/ledger-excel-cleanup-20260922-baseline/。Excel工作簿、历史快照、图片导出、账务仓储、权限、内容安全和隐私逻辑未改。
- 没有部署ledger或GC、没有上传触发器/体验版、没有提交审核、没有修改存储权限或正式数据库、没有清理现有三份云文件、没有提交或推送本轮Git改动。后续生效需要部署ledger及GC、在微信开发者工具右键GC“上传触发器”、验证临时身份权限、上传小程序并真机复测；不需要数据库迁移。


## 2026-09-22 客户负责人/当前账期负责人自定义规格创建与底槽识别（本地完成，未部署）

- 原 createCustomProduct 在 ledger 管理员 action 与 member-security 门禁中只准 admin，领域创建函数没有客户范围判断，快速记账创建按钮也未按负责人过滤。现创建独立授权为真实 active admin 或 client owner 或唯一真实 open period owner；其他产品写 action 门禁保持原样。
- 云事务按真实 OPENID 重新获取 membership/tenant，普通成员必须提交 clientId，由事务定向读取真实客户及 tenantId+clientId+status=open（最多2条）。没有/多个open时不使用账期分支，admin和client owner仍可创建；不从历史/合成账期、前端periodId或权限字段授权。补齐当前成员到事务快照，避免通用1000条读取上限漏掉授权成员。
- 产品仍然tenant共享，只把clientId用于请求授权，不新增产品归属字段。普通成员重复create只返回已有产品，不合并aliases/recognitionKeywords/specialTags/基础规格/颜色/齿型/长度/名称等；既有创建审计和msgSecCheck保持，operator为真实成员。创建路径使用preserveShipments读取，避免顺带回填旧发货字段。
- 底槽原来不在特殊词表，导致后续仅按尺寸/颜色/齿型匹配普通标准品。现进入既有specialTags流程，建议名称与normalizedName保留底槽；名称/别名/关键词/结构化复用和去重都先校验底槽边界及其长度/特殊标签兼容性。底槽+1.3米长+100根正确分离；普通4025和底槽4025不能互相自动匹配。
- 提取现有确定性尺寸/颜色/齿型/长度函数为services/product-identity.js并同步云副本；原标准产品事实数据、normalizeCustomProductName函数和非底槽匹配行为保持。已有底槽名称但空specialTags可在新输入匹配时只读识别，不批量重写旧产品，更不改历史成交快照。
- 快速记账按客户显示创建入口，切换客户清除旧表单，保存传入客户上下文，防重复提交；底槽与普通规格在选择器分开。原始普通行只有经用户在新建表单明确编辑并保存新底槽身份后才采用新产品。全局产品页和直接表单路由补管理员页面守卫，未给成员产品管理权限。
- 新增108项专项测试（权限/真实云入口模拟/事务冲突/内容安全/解析/去重及13项页面串联）；全量npm test 558/558通过，npm run check通过24页、112 JS、24 WXML、25 WXSS微信原生编译，git diff --check通过。原发货修正测试只将create移出旧admin-only断言，新专项测试完整覆盖新门禁。
- 修改前219文件基线在/private/tmp/ledger-custom-bottom-20260922-baseline/，已有Excel清理未提交改动完整保留。价格、saveAsDefault、发货修正、结清、删除、成员、转移、审计等23个保护函数与基线逐字一致；Excel/图片/cleanup/GC、内容安全、隐私与产品事实文件未改。
- 本轮没有部署ledger、上传体验版、提交审核、修改正式数据库/云存储权限、提交或推送Git。后续生效需重新部署ledger并上传体验版，再进行真实微信/iPhone验收；本轮测试使用本地内存数据库事务模拟，不代表正式云端并发或真机已验证。不需要数据库迁移、集合/字段/索引或依赖变更。


## 2026-09-22 最新源码 Git 提交（本地已提交，公开推送待确认）

- 用户要求“现在把最新的代码提交到git仓库”。已在原仓库main创建源码提交 `cb39e4c71103ceada7300b00562a29925424578c`，标题为 `fix: scope custom product creation, preserve bottom grooves and clean exports`，覆盖38个已核验文件，包含本轮权限/底槽修复及此前未提交的Excel cleanup/48小时GC与测试、报告。
- 提交前重新执行npm test：558/558通过；npm run check通过24页面、112 JS、24 WXML、25 WXSS原生编译；git diff --cached --check通过。38个文件约665KB，常见私钥、GitHub/API/云密钥模式无命中；本机配置与node_modules仍按原.gitignore排除。
- 已联网确认远端main与提交前本地基线均为 `45dd04988c687b1b4f9a1d5979c91ce1442e3b62`。执行 `git push origin main` 时自动审批拒绝：公开GitHub目的地需明确用户授权，当前“提交到git仓库”未被审批视为足够的公开推送授权；命令未执行，没有绕过或改用其它上传路径。
- 当前成果为本地Git提交；需用户明确确认将最新提交推送到公开仓库 `https://github.com/tuopro/jizhang` 的main后再继续正常推送。没有部署云函数、上传体验版、提交微信审核、修改正式数据库或云存储权限。


## 2026-09-22 最新代码推送 jizhang（已完成）
...
## 2026-09-22 邀请小程序码及按钮修复推送 GitHub（已完成）

- 用户明确要求将当前小程序上传到其GitHub仓库。核对原仓库为https://github.com/tuopro/jizhang.git、分支main，推送前本地与远端均为a1ee47e4595d984e7e9b3b540086254af2fcef24；正常快进推送，没有强推或改写历史。
- 创建并成功推送源码提交a709cd22081df52c1a6609d8dbc38997de0b3491，标题feat: add member invitation mini-program codes and render checks。22个文件包含二维码生成、scene恢复及原邀请安全流程、前端包含项修复、页面/云端测试和两轮报告；GitHub返回a1ee47e..a709cd2 main -> main。
- 上传前重新运行npm test：656/656通过；npm run check：24页面、117 JS、24 WXML、25 WXSS原生编译通过；git diff --cached --check通过。候选22文件约227KB，常见私钥/GitHub/API/云密钥模式扫描无命中；project.private.config.json、node_modules等继续排除。
- 本轮只完成Git源码同步及进度记录，没有部署云函数、上传微信体验版、提交微信审核或修改正式数据库/存储权限。二维码按钮在手机体验版生效仍需按前一轮报告重新上传修复后的前端包。
